...
<%
    ...
    if ("editar".equals(request.getParameter("cmd"))) {
        ...
        if (codigo != null && productoId != null && fechaIngreso != null && fechaVencimiento != null
                && almacenCodigo != null && costoStr != null && observaciones != null
                && !codigo.isEmpty() && !productoId.isEmpty() && !fechaIngreso.isEmpty()
                && !fechaVencimiento.isEmpty() && !almacenCodigo.isEmpty() && !costoStr.isEmpty()) {
    ...
    if ("guardar".equals(request.getParameter("cmd"))) {
        ...
        if (codigo != null && productoId != null && cantidad != null && almacenCodigo != null && accion != null
                ...
                if (loteDAO.existeLote(codigo)) {
                    ...
                    switch (accion) {
                        case "retiro":
                            ...
                            TipoMovimiento tipo = TipoMovimientoDAO.buscarPorCodigo("TM05");
                            // Crear el movimiento usando el constructor COMPLETO
                            MovimientoInventario movRetiro = new MovimientoInventario(
                                    codigoMovimientoR,     // codigo
                                    tipo,                   // tipoMovimiento
                                    codigo,                // afectado (código del lote)
                                    Integer.parseInt(cantidad),           // cantidad (negativa)
                                    0.0,                   // costoMovimiento
                                    "N/A",                 // antiguaUbicacion → se aplica N/A
                                    "N/A",                 // nuevaUbicacion   → se aplica N/A
                                    new java.util.Date(),   // fecha
                            ...
                            break;
                        case "retiroerror":
                            String codigoMovimientoRE = "MV" + UUID.randomUUID()
                                    .toString()
                                    .replace("-", "")
                                    .substring(0, 8);

                            TipoMovimiento tipoRE = TipoMovimientoDAO.buscarPorCodigo("TM07");
                            // Crear el movimiento usando el constructor COMPLETO
                            MovimientoInventario movRetiroError = new MovimientoInventario(
                                    codigoMovimientoRE,     // codigo
                                    tipoRE,                   // tipoMovimiento
                                    codigo,                // afectado (código del lote)
                                    Integer.parseInt(cantidad),           // cantidad (negativa)
                                    0.0,                   // costoMovimiento
                                    "N/A",                 // antiguaUbicacion → se aplica N/A
                            ...
                            movimientoDAO.insertarMovimiento(movRetiroError);
                            controlador = -Integer.parseInt(cantidad);
                            break;

                        case "ingreso":
                            String codigoMovimientoI = "MV" + UUID.randomUUID()
                                    .toString()
                                    .replace("-", "")
                                    .substring(0, 8);

                            TipoMovimiento tipoI = TipoMovimientoDAO.buscarPorCodigo("TM01");
                            // Crear el movimiento usando el constructor COMPLETO
                            MovimientoInventario movIngreso = new MovimientoInventario(
                                    codigoMovimientoI,     // codigo
                                    tipoI,                   // tipoMovimiento
                                    codigo,                // afectado (código del lote)
                                    Integer.parseInt(cantidad),           // cantidad (negativa)
                                    0.0,                   // costoMovimiento
                                    "N/A",                 // antiguaUbicacion → se aplica N/A
                            ...
                            movimientoDAO.insertarMovimiento(movIngreso);
                            controlador = Integer.parseInt(cantidad);
                            break;
                        case "ingresoerror":
                            String codigoMovimientoIE = "MV" + UUID.randomUUID()
                                    .toString()
                                    .replace("-", "")
                                    .substring(0, 8);

                            TipoMovimiento tipoIE = TipoMovimientoDAO.buscarPorCodigo("TM08");
                            // Crear el movimiento usando el constructor COMPLETO
                            MovimientoInventario movIngresoError = new MovimientoInventario(
                                    codigoMovimientoIE,     // codigo
                                    tipoIE,                   // tipoMovimiento
                                    codigo,                // afectado (código del lote)
                                    Integer.parseInt(cantidad),           // cantidad (negativa)
                                    0.0,                   // costoMovimiento
                                    "N/A",                 // antiguaUbicacion → se aplica N/A
                            ...
                            movimientoDAO.insertarMovimiento(movIngresoError);
                            controlador = Integer.parseInt(cantidad);
                            break;
                    }
                    ...
                    if (producto == null) {
                        ...
                        if (actualizado) {
                            mensaje = "<strong style='color:green;'>Lote actualizado correctamente. acción: "+ accion +"</strong>";
                        } else {
                            mensaje = "<strong style='color:red;'>No se pudo actualizar el lote</strong>";
                        }
                    }
            ...
            mensaje = "<strong style='color:orange;'>Debe ingresar todos los campos solicitados "+ request.getParameter("cmd") + codigo +"</strong>";
        }
    }
