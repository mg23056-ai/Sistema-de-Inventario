...
<html>
    <head>
        ...
        <style>
            ...
            select[name="tipo"] { background-color: #34495e; color: white; border: none; border-radius: 8px; padding: 8px 12px; font-size: 14px; width: 180px; cursor: pointer; transition: all 0.3s ease; outline: none; }
            select[name="tipo"]:focus { background-color: #3e5871; box-shadow: 0 0 5px #1abc9c; }
            select[name="tipo"] option { background-color: #2c3e50; color: white; }
    ...
    <body>
        ...
        <div>
            <%
                // Recibir filtro desde la URL
                String filtroTipo = request.getParameter("tipo");

                // Obtener lista de tipos para el select
                List<TipoMovimiento> tipos = tipoMovimientoDAO.listarTipos();

                // Lista de movimientos según filtro
                List<MovimientoInventario> movimientos;
                if (filtroTipo != null && !filtroTipo.trim().isEmpty()) {
                    movimientos = movimientoDAO.listarMovimientosFiltrado(registrosPorPagina, offset, filtroTipo);
                } else {
                    movimientos = movimientoDAO.listarMovimientos(registrosPorPagina, offset);
                }
            %>
                ...
                <select name="tipo">
                    ...
                    <% for (TipoMovimiento t : tipos) { %>
                        <option value="<%= t.getCodigo() %>"
                            <%= (filtroTipo != null && filtroTipo.equals(t.getCodigo())) ? "selected" : "" %>>
                            <%= t.getNombre() %>
                        </option>
                    <% } %>
                </select>
            ...
            <form method="get" action="exportarMovimientos.jsp">
                <input type="hidden" name="tipo" value="<%= (filtroTipo != null ? filtroTipo : "") %>">
                <button type="submit">Descargar Tabla</button>
            </form>
            ...
            <div class="container">
                <table class="tabla-mov">
                    ...
                    <tbody>
                            ...
                            try {
                                if (movimientos != null && !movimientos.isEmpty()) {
                                    ...
                                    for (MovimientoInventario m : movimientos) {
                                        ...
                                        String id = (actorCrudo != null ? actorCrudo.trim() : "");
                        ...
                        <tr>
                            <td><%= m.getCodigo() %></td>
                            <td><%= (m.getTipoMovimiento() != null ? m.getTipoMovimiento().getNombre() : "Sin tipo") %></td>
                ...
                <div class="paginacion">
                    <div style="margin-bottom:10px;">
                        ...
                        %>
                                    <a href="?pagina=<%= i %><%= (filtroTipo != null ? "&tipo=" + filtroTipo : "") %>" 
                                       style="margin:0 5px; color:#007bff; text-decoration:none;"><%= i %></a>
                        <%
                                }
                            }
                        %>
                    ...
                    <div>
                        <%
                            if (pagina > 1) {
                        %>
                                <a href="?pagina=<%= pagina - 1 %><%= (filtroTipo != null ? "&tipo=" + filtroTipo : "") %>" 
